...
| **앨범 암호화 (선택)** | 새 앨범을 만들 때 **암호화**를 켜고 앨범 암호를 정하면, 사진·영상·썸네일·앨범 정보가 모두 기기에서 암호화된 뒤 올라가요. GitHub에서는 이름·날짜·장소·사진 내용을 볼 수 없어요 |
...
암호화는 **새 앨범을 만들 때** 켤 수 있어요 (이미 있는 앨범을 암호화로 바꾸는 기능은 아직 없어요).
...
- **멤버 초대**: 초대는 평소처럼 하고, **앨범 암호는 따로 알려주세요** (메신저 등). 초대받은 사람은 앨범을 처음 열 때 암호를 입력해요. **이 기기에서 기억하기**를 켜면 다음부터는 묻지 않아요 (키는 꺼낼 수 없는 형태로 IndexedDB에 저장돼요).
...
- **암호를 잃어버리면 복구할 수 없어요.** 저도, GitHub도 풀 수 없어요.
