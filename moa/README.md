...
- **암호 바꾸기**: 설정 → **암호 바꾸기**. 사진을 다시 암호화하지 않고 키만 새 암호로 감싸요. 예전 암호로 감싼 키는 기본으로 기록에서도 지워요. 다만 예전 암호를 알던 사람이 키를 이미 꺼내뒀다면 막을 수 없어서, 멤버를 내보낼 때 완전히 차단하려면 새 암호화 앨범으로 옮겨야 해요.
...
- **암호를 잊었을 때**
  - **복구 코드**: 암호화할 때 `XXXX-XXXX-XXXX-XXXX-XXXX-XXXX` 코드를 한 번 보여줘요 (복사·파일로 저장). 잠긴 화면의 **암호를 잊으셨나요?**에 입력하면 열리고, 바로 새 암호를 정해요. 설정 → **복구 코드**에서 새로 만들 수 있어요 (예전 코드는 무효).
  - **열려 있는 기기**: 이 앨범이 열려 있는 멤버의 기기(기억하기를 켠 기기)에서는 설정 → 암호 바꾸기 → **암호를 잊으셨나요?**로 예전 암호 없이 새 암호를 정할 수 있어요.
