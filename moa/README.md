...
| **라이브 포토** | `IMG_1234.HEIC` + `IMG_1234.MOV`를 같이 올리면 자동으로 하나로 묶어요 (Apple 라이브 포토 ID → 파일 이름 → 촬영 시각 순으로 매칭). 사진을 열면 한 번 움직이고, **길게 누르면** 소리와 함께 재생돼요. 사진 보관함에서 바로 고르면 iOS가 정지 사진만 넘겨줘서 업로드 창에 방법을 안내하고, 나중에 사진 정보 → **라이브 영상 붙이기**로 움직임을 더할 수 있어요 |
...
| **태그 · 빠른 기호** | 사진 정보의 **+ 추가**를 누르면 태그 편집이 열리고 전에 쓴 태그를 보여줘요. 여러 장 선택해서 달 수도 있어요. ⭐ 📌 ✈️ 🍽️ 같은 **빠른 기호**는 한 번 눌러 달고(♥처럼 썸네일에 표시), "여행"을 치면 ✈️처럼 어울리는 기호와 쓰던 태그를 추천해요. 앨범에서 자주 쓰는 기호가 앞에 와요. 태그별 보기, 칩으로 필터, 검색(태그·장소·설명·올린 사람) |
...
| **공유앨범** | 친구를 저장소 협업자로 초대하면 같은 보관함을 함께 채워요. 앨범 만들기, 날짜·장소 직접 수정. **좋아요**는 하트만 보이고 꾹 누르면 누가 눌렀는지 떠요. **캡션**은 따로 쓰지 않고 댓글을 **고정**하면 사진 아래 캡션이 돼요 |
...
| **확대** | 사진을 두 손가락으로 벌리거나 **두 번 탭**해서 확대, 확대한 채로 끌어서 이동 (컴퓨터는 트랙패드 핀치·Ctrl+휠) |
| **저장 실패 대처** | 상단의 **저장 실패**를 누르면 GitHub가 보낸 이유를 보여주고, 다시 시도·내용 복사·저장 안 된 변경 버리기를 할 수 있어요. 같은 변경을 GitHub가 계속 거절하면 그 하나만 건너뛰고 나머지를 저장해요 |
