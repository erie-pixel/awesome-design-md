...
| **완전 삭제** | 삭제할 때 **기록에서도 완전히 지우기**를 켜거나 설정 → **기록 완전 삭제**를 누르면, Git 이력을 지금 파일만 담은 커밋 1개로 바꿔서 지운 사진이 어떤 커밋에도 남지 않아요 |
...
- **새 앨범**: 만들 때 **암호화**를 켜요.
- **쓰던 앨범**: 설정 → **이 앨범 암호화하기** (주인만). 모든 파일을 받아서 암호화해 다시 올린 뒤, 한 번에 바꾸고 암호화 전 사본을 기록에서 지워요. 사진이 많으면 오래 걸리고, 중간에 앱을 닫아도 설정에서 **이어서 하기**로 계속할 수 있어요. 바꾸는 동안 친구들은 평소처럼 써도 되고, 그 사이 올라온 사진도 같이 암호화돼요. 끝나면 멤버들은 암호를 한 번 입력해야 해요.
- **끄기**: 설정 → **암호화 끄기** (주인만). 반대로 암호를 풀어 날짜별 폴더로 다시 저장해요.
...
- **암호 바꾸기**: 설정 → **암호 바꾸기** (앨범 주인만). 사진을 다시 암호화하지 않고 키만 새 암호로 감싸요. 예전 암호로 감싼 키는 기본으로 기록에서도 지워요. 다만 예전 암호를 알던 사람이 키를 이미 꺼내뒀다면 막을 수 없어서, 멤버를 내보낼 때 완전히 차단하려면 새 암호화 앨범으로 옮겨야 해요.
- **Face ID · Touch ID로 열기**: 설정 → **Face ID로 열기** → 켜기. 이 기기에 앨범용 패스키를 만들고, 패스키의 PRF 확장이 주는 비밀값(HKDF → AES-GCM)으로 앨범 키를 한 번 더 감싸서 `album.json`에 넣어요. 잠긴 화면에서 **Face ID로 열기**를 누르면 암호 없이 열려요.
